...
def trainNetwork(inputVector,target):
    ...
    for sampleNo in range(len(inputVector)):
            ...
            for j in range(len(target[int(sampleNo/3)])):
                    if Y[j] != target[int(sampleNo/3)][j]:
                        b[j]=b[j]+target[int(sampleNo/3)][j]
                        for i in range(len(inputVector[sampleNo])):
                            weight[j][i] = weight[j][i]+ target[int(sampleNo/3)][j]*inputVector[sampleNo][i]
...
def testNetwork(input,weights,b,inputCharacter):
    ...
    if len(indexOfOnes) == 1:
        ...
        if len(indexOfOnes)  > 1 :
            min = y_in[indexOfOnes[0]]
            minIndex = 0
            for k in range(len(indexOfOnes)):
                if abs(y_in[indexOfOnes[k]] - teta)<= min:
                    minIndex =indexOfOnes[k]
            try:
                if characters[minIndex]==inputCharacter:
                    return True
                else:
                    return False
            except:
